...
export default function FunClubConcert() {
    ...
    useEffect(() => {
        const loadImages = async () => {
            try {
                const urls = await imageService.getFunkClubConcertImages();
                setImageUrls(urls);
            } catch (err) {
    ...
    return (
        <>
            <div className="flex items-center justify-center text-white mt-22">
                <h1 className="text-5xl font-bold mb-6">Funk club concert</h1>
            </div>
            <div className="text-white py-6 px-4 min-h-screen">
                <ConcertGallery images={imagesUrls} altPrefix="Concierto funk club" />
            </div>
        </>
    )
}