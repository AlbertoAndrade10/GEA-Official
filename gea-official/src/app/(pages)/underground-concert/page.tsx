...
export default function UndergroundConcert() {
    ...
    useEffect(() => {
        const loadImages = async () => {
            try {
                const urls = await imageService.getUndergroundConcertImages()
                setImageUrls(urls);
            } catch (err) {
    ...
    return (
        <>
            <div className="flex items-center justify-center text-white mt-22">
                <h1 className="text-5xl font-bold mb-6">Underground concert</h1>
            </div>
            <div className="text-white py-6 px-4 min-h-screen">
                <ConcertGallery images={imagesUrls} altPrefix="Concierto underground" />
            </div>
        </>
    )
}